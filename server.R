...
shinyServer(function(input, output) {
  ...
  output$bigrams <- renderPlot({
    ...
    if(length(words) >= 1) {
      ...
      if(!is.na(results$o1[1])) {
        ...
        biplot <- ggplot(results, aes(x=o1, y=n)) + geom_bar(stat="identity") +
          theme_bw() + coord_flip() +
          labs(title="Frequent words based on unigram", x="Predicted words", y="Count")
        return(biplot)  
      }
    }
  })
  output$trigrams <- renderPlot({
    ...
    if(length(words) >=2) {
      ...
      if(!is.na(results$o1[1])) {
        results$o1 <- factor(results$o1, levels = arrange(results, n)$o1)
        triplot <- ggplot(results, aes(x=o1, y=n)) + geom_bar(stat="identity") +
          theme_bw() + coord_flip() +
          labs(title="Frequent words based on bigram", x="Predicted words", y="Count")
        return(triplot)
      }
  ...
  output$fourgrams <- renderPlot({
    ...
    if(length(words) >=3) {
      ...
      if(!is.na(results$o1[1])) {
        ...
        triplot <- ggplot(results, aes(x=o1, y=n)) + geom_bar(stat="identity") +
          theme_bw() + coord_flip() +
          labs(title="Frequent words based on trigram", x="Predicted words", y="Count")
        return(triplot)
      }
    }
  })
